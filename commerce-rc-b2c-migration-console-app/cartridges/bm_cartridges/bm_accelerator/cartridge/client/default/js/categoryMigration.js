...
_APP.init = function () {
    ...
    _APP.post(_APP.STATUS_URL, '', function (data) { if (!data.ok) return; var status = data.status || {}; var missing = Object.keys(status).filter(function (k) { return status[k] === 'missing'; }); var infoBox = document.getElementById('step1-info-box'); var skipBtn = document.getElementById('btn-skip-attrs'); if (!missing.length) { if (infoBox) infoBox.textContent = 'Step 1 - All 3 custom attributes (ctSlug, ctId, ctPosition) already exist. You may skip.'; if (skipBtn) { skipBtn.textContent = 'All exist - Skip to Step 2'; skipBtn.style.background = '#e8f5e9'; skipBtn.style.color = '#2e7d32'; } } else { if (infoBox) infoBox.textContent = 'Step 1 - Missing: ' + missing.join(', ') + '. Click Check and Create.'; } });
};