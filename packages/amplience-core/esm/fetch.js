import { buildCdnUrl, buildCdnUrlById } from './cdn.js';
import { toRendererModel } from './transform.js';
/**
...
function fetchLiveContent(hubName, deliveryKey, options) {
    ...
    var url = buildCdnUrl(hubName, deliveryKey);
    if (!url && options.contentId) {
        url = buildCdnUrlById(hubName, options.contentId);
    }
    if (!url) {
        ...
        .then(function (res) {
            if (res.ok) {
                return res.json().then(function (payload) {
                    ...
                    var model = toRendererModel(content, {
                        hubName: hubName,
            ...
            if (res.status === 404 && options.contentId && deliveryKey) {
                var idUrl = buildCdnUrlById(hubName, options.contentId);
...
export { unwrapCdnPayload, fetchLiveContent };
